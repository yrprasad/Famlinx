 ...
 */
public class MapAreaManager implements OnMarkerDragListener, GoogleMap.OnMarkerClickListener {
    ...
    private int cancelDrawableId = -1;
    ...
    public interface CircleManagerListener {
         ...
         * @param draggableCircle created circle
         */
        void onDeleteCircle(MapAreaWrapper draggableCircle);
    ...
    public MapAreaManager(GoogleMap map, int strokeWidth, int strokeColor, int circleColor,
                          int moveDrawableId, int resizeDrawableId, int cancelDrawableId, float moveDrawableAnchorU, float moveDrawableAnchorV, float resizeDrawableAnchorU, float resizeDrawableAnchorV,
...
//        map.setOnMapLongClickListener(this);
    ...
    public MapAreaManager(GoogleMap map, int strokeWidth, int strokeColor, int circleColor,
        ...
        this(map, strokeWidth, strokeColor, circleColor, -1, -1, -1, initRadius, circleManagerListener);
    }
     ...
     */
    public MapAreaManager(GoogleMap map, int strokeWidth, int strokeColor, int circleColor,
                          int moveDrawableId, int radiusDrawableId, int cancelDrawableId,
                          MapAreaMeasure initRadius, CircleManagerListener circleManagerListener) {

        this(map, strokeWidth, strokeColor, circleColor, moveDrawableId, radiusDrawableId, cancelDrawableId, 0.5f, 1f, 0.5f, 1f, initRadius, circleManagerListener);
    }
    ...
    public void createFence(LatLng point) {
        ...
        MapAreaWrapper circle = new MapAreaWrapper(map, point, initRadiusMetersFinal, strokeWidth, strokeColor, fillColor, minRadiusMeters, maxRadiusMeters,
                moveDrawableId, radiusDrawableId, cancelDrawableId, moveDrawableAnchorU, moveDrawableAnchorV, resizeDrawableAnchorU, resizeDrawableAnchorV);
    ...
    public boolean onMarkerClick(Marker marker) {
        ...
        MapAreaWrapper.MarkerClickResult result = MapAreaWrapper.MarkerClickResult.none;
        MapAreaWrapper affectedDraggableCircle = null;

        for (MapAreaWrapper draggableCircle : areas) {
            result = draggableCircle.onMarkerClicked(marker);
            if (result != MapAreaWrapper.MarkerClickResult.none) {
                affectedDraggableCircle = draggableCircle;
                break;
            }
        }

        if(affectedDraggableCircle != null)
        {
            affectedDraggableCircle.deleteCircle();
            areas.remove(affectedDraggableCircle);
            circleManagerListener.onDeleteCircle(affectedDraggableCircle);
            return true;
        }
